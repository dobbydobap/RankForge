...
const VERDICT_COLORS: Record<string, string> = {
  ...
  RUNTIME_ERROR: '#7a7a82', COMPILATION_ERROR: '#9a9aa4',
  PENDING: '#71717a', JUDGING: '#b4b4bc', MEMORY_LIMIT_EXCEEDED: '#a8a8b2',
};
      ...
      <main className="flex-1 w-full px-6 lg:px-10 py-8">
        ...
        <div className="border border-[var(--c-border-2)] rounded-xl bg-[var(--c-surface)] p-4">
          ...
          <input
            ...
            className="w-full max-w-md px-3 py-2 mb-4 bg-rf-dark border border-rf-iron rounded-lg text-[var(--c-fg)] text-sm placeholder-rf-iron focus:outline-none focus:ring-2 focus:ring-orange-500"
          />
          ...
          {users?.users && (
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-b border-[var(--c-border-2)]">
                ...
                <tbody className="divide-y divide-[var(--c-border-2)]">
                  {users.users.map((u: any) => (
                    <tr key={u.id} className="hover:bg-[var(--c-surface)]">
                      <td className="px-3 py-2 text-sm text-[var(--c-fg)]">{u.username}</td>
                      <td className="px-3 py-2 text-sm text-rf-gray">{u.email}</td>
                      <td className="text-center px-3 py-2">
                        <span className="px-2 py-0.5 text-xs bg-[var(--c-surface-3)] text-orange-400 rounded">{u.role}</span>
                      </td>

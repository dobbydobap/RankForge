...
export default function ProblemDetailPage() {
  ...
  return (
    <div className="h-screen flex flex-col bg-rf-black">
      {/* Top bar */}
      <div className="flex items-center justify-between px-4 h-12 border-b border-[var(--c-border-2)] bg-rf-black/80 backdrop-blur-sm shrink-0">
        <div className="flex items-center gap-4">
          <Link href="/problems" className="text-sm text-rf-gray hover:text-[var(--c-fg)]">&larr; Problems</Link>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="px-4 py-1.5 text-sm font-medium border border-[var(--c-border-2)] hover:border-rf-iron disabled:opacity-50 text-rf-light rounded-lg transition-colors"
          >
          ...
          <button
            ...
            className="px-4 py-1.5 text-sm font-medium bg-[var(--c-fg)] hover:opacity-80 disabled:opacity-50 text-rf-black rounded-lg transition-colors"
          >
      ...
      <div className="flex-1 flex min-h-0">
          ...
          <div className="flex border-b border-[var(--c-border-2)] shrink-0">
            {(['problem', 'submissions'] as LeftTab[]).map((t) => (
              <button key={t} onClick={() => setLeftTab(t)}
                className={`px-4 py-2 text-sm font-medium capitalize transition-colors ${leftTab === t ? 'text-[var(--c-fg)] border-b-2 border-[var(--c-fg)]' : 'text-rf-gray hover:text-[var(--c-fg)]'}`}
              >{t}</button>
            ))}
          </div>
              ...
              <div className="space-y-2">
                {!submissionsData?.submissions.length ? (
                  ...
                  <Link key={sub.id} href={`/submissions/${sub.id}`}
                    className="flex items-center justify-between p-3 rounded-lg border border-[var(--c-border-2)] bg-[var(--c-surface)] hover:border-rf-iron transition-colors">
        ...
        <div className={`${focusMode ? 'w-full' : 'w-[55%]'} flex flex-col min-h-0`}>
          ...
          {/* Bottom panel */}
          <div className="h-[38%] border-t border-[var(--c-border-2)] flex flex-col shrink-0">
            <div className="flex border-b border-[var(--c-border-2)] shrink-0 px-2">
              {(['testcases', 'result'] as BottomTab[]).map((t) => (
                <button key={t} onClick={() => setBottomTab(t)}
                  className={`px-3 py-2 text-xs font-medium capitalize transition-colors ${bottomTab === t ? 'text-[var(--c-fg)] border-b-2 border-[var(--c-fg)]' : 'text-rf-gray hover:text-[var(--c-fg)]'}`}
                >
                  ...
                  <div className="flex gap-2 mb-3">
                    {sampleCases.map((_: any, i: number) => (
                      <button key={i} onClick={() => setActiveCase(i)}
                        className={`px-3 py-1 text-xs rounded-lg transition-colors ${activeCase === i ? 'bg-[var(--c-surface-3)] text-[var(--c-fg)]' : 'text-rf-gray hover:text-[var(--c-fg)]'}`}
                      >Case {i + 1}</button>
                    ))}
                  </div>
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        <textarea
                          ...
                          className="w-full h-20 px-3 py-2 bg-rf-dark border border-[var(--c-border-2)] rounded-lg text-[var(--c-fg)] text-xs font-mono resize-none focus:outline-none focus:ring-1 focus:ring-rf-iron"
                        />
                      </div>
                      <div>
                          ...
                          return (
                            <button key={i} onClick={() => setActiveCase(i)}
                              className={`flex items-center gap-1 px-3 py-1 text-xs rounded-lg transition-colors ${
                                activeCase === i ? 'bg-[var(--c-surface-3)] text-[var(--c-fg)]' : 'text-rf-gray hover:text-[var(--c-fg)]'
                              }`}
                            >

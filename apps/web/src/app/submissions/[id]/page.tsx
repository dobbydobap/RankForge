    ...
    <>
      <main className="flex-1 w-full px-6 lg:px-10 py-8">
        ...
        {submission.testResults.length > 0 && (
          <div className="mb-6">
            <h2 className="text-lg font-semibold text-[var(--c-fg)] mb-3">Test Results</h2>
            <div className="flex flex-wrap gap-2">
              {submission.testResults.map((tr: any, idx: number) => (
                <div
                  key={idx}
                  className="flex items-center gap-2 px-3 py-2 border border-[var(--c-border-2)] rounded-lg bg-[var(--c-surface)]"
                >
        ...
        {/* Source Code */}
        <div>
          <h2 className="text-lg font-semibold text-[var(--c-fg)] mb-3">Source Code</h2>
          <div className="h-96">
            <CodeEditor
...
function StatCard({ label, value }: { label: string; value: string }) {
  return (
    <div className="p-3 border border-[var(--c-border-2)] rounded-lg bg-[var(--c-surface)]">
      <p className="text-xs text-rf-gray">{label}</p>
      <p className="text-sm font-medium text-[var(--c-fg)] mt-0.5">{value}</p>
    </div>
  );
}

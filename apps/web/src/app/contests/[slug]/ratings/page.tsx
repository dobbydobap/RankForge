  ...
  return (
    <>
      <main className="flex-1 w-full px-6 lg:px-10 py-8">
        <div className="mb-6">
          <h1 className="font-display text-3xl sm:text-4xl uppercase tracking-tight text-[var(--c-fg)]">Rating Changes</h1>
          <Link
            href={`/contests/${slug}`}
          ...
          <div className="border border-[var(--c-border-2)] rounded-xl overflow-hidden">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-[var(--c-border-2)]">
                {changes.map((c: any) => (
                  <tr key={c.userId} className="hover:bg-[var(--c-surface)] transition-colors">
                    ...
                    <td className="text-center px-4 py-3 text-sm font-medium text-[var(--c-fg)]">
                      {c.newRating}
                    </td>

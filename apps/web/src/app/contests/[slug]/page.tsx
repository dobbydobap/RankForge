          ...
          <div className="text-right">
            ...
            {!contest.isRegistered && !isEnded && (
              <button
                ...
                className="mt-3 px-5 py-2 text-xs uppercase tracking-wide font-medium bg-[var(--c-fg)] hover:opacity-80 disabled:opacity-50 text-[var(--c-bg)] transition-colors"
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-[var(--c-fg)]">Problems</h2>
              {(isLive || isEnded) && (
                <Link
                  href={`/contests/${slug}/leaderboard`}
                  className="text-sm text-orange-400 hover:text-[var(--c-fg)] transition-colors"
                >
            ...
            {!showProblems ? (
              <div className="p-6 border border-[var(--c-border-2)] rounded-xl bg-[var(--c-surface)] text-center text-sm text-rf-gray">
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-[var(--c-border-2)]">
                    {contest.problems.map((p: any) => (
                      <tr key={p.label} className="hover:bg-[var(--c-surface)] transition-colors">
          ...
          <div className="space-y-4">
            ...
            <div className="border border-[var(--c-border-2)] rounded-xl bg-[var(--c-surface)] p-4">
              <h3 className="text-sm font-semibold text-[var(--c-fg)] mb-3">Announcements</h3>
              {contest.announcements.length === 0 ? (
                <p className="text-xs text-rf-gray">No announcements yet.</p>
              ) : (
                <div className="space-y-2">
                  {contest.announcements.map((a: any) => (
                    <div key={a.id} className="p-2 bg-[var(--c-surface-3)] rounded-lg">
            ...
            {/* Contest Info */}
            <div className="border border-[var(--c-border-2)] rounded-xl bg-[var(--c-surface)] p-4">
              <h3 className="text-sm font-semibold text-[var(--c-fg)] mb-3">Contest Rules</h3>

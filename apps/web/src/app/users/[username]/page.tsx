...
export default function UserProfilePage() {
  ...
  const ratingColor =
    ...
    (user.profile?.currentRating ?? 1200) >= 1200 ? 'text-orange-400' : 'text-rf-gray';
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-6">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-4">
              <div className="w-20 h-20 rounded-full bg-[var(--c-surface-3)] border-2 border-rf-iron flex items-center justify-center text-2xl font-bold text-rf-gray shrink-0">
            ...
            <div className="border border-[var(--c-border-2)] rounded-xl bg-[var(--c-surface)] p-4">
              ...
              <div className="space-y-2.5">
                ...
                <StatRow label="Problems Solved" value={String(stats.totalSolved)} color="text-orange-400" />
              </div>
            </div>
                ...
                <div className="space-y-2">
                  {stats.languages.map((l: any) => (
                    <div key={l.language} className="flex items-center justify-between">
                      <span className="px-2 py-0.5 text-xs bg-[var(--c-surface-3)] text-orange-400 rounded">
                ...
                <div className="space-y-1.5">
                  {stats.skills.slice(0, 8).map((s: any) => (
                    <div key={s.name} className="flex items-center gap-2">
                      ...
                      <div className="flex-1 h-1.5 bg-[var(--c-surface-3)] rounded-full overflow-hidden">
                        <div
          ...
          <div className="space-y-4">
            ...
            <div className="border border-[var(--c-border-2)] rounded-xl bg-[var(--c-surface)] p-5">
              <div className="flex items-center gap-8">
                ...
                <div className="relative w-32 h-32 shrink-0">
                  <svg viewBox="0 0 36 36" className="w-full h-full -rotate-90">
                    ...
                    <circle
                      cx="18" cy="18" r="15.9" fill="none" stroke="#8a8a98"
                      strokeWidth="3" strokeDasharray={`${solvePercent} ${100 - solvePercent}`}
                      strokeLinecap="round"
                    />
                  </svg>
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <span className="text-2xl font-bold text-[var(--c-fg)]">{stats.totalSolved}</span>
                ...
                <div className="flex-1 grid grid-cols-2 gap-3">
                  {['EASY', 'MEDIUM', 'HARD', 'EXPERT'].map((d) => (
                    <div key={d} className="flex items-center justify-between px-3 py-2 border border-[var(--c-border-2)] rounded-lg">
            ...
            <div className="border border-[var(--c-border-2)] rounded-xl bg-[var(--c-surface)] p-5">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-semibold text-orange-400">
                  <span className="text-[var(--c-fg)] font-bold">{stats.totalSubmissions}</span>{' '}
                  submissions in the past one year
                </h3>
                <div className="flex items-center gap-4 text-xs text-rf-gray">
                  <span>Total active days: <span className="text-orange-400">{stats.activeDays}</span></span>
                  <span>Max streak: <span className="text-orange-400">{stats.maxStreak}</span></span>
                </div>
              </div>
                  ...
                  {Array.from({ length: 53 }, (_, weekIdx) => (
                    <div key={weekIdx} className="flex flex-col gap-[3px]">
                      {Array.from({ length: 7 }, (_, dayIdx) => {
                        ...
                        const colors = ['bg-[var(--c-surface-2)]', 'bg-[var(--c-border-2)]', 'bg-[var(--c-line)]', 'bg-orange-700', 'bg-orange-500'];
                        return (
                          <div
                            key={dayIdx}
              ...
              <div className="flex items-center gap-1.5 mt-2 text-[10px] text-rf-gray justify-end">
                <span>Less</span>
                <div className="w-[11px] h-[11px] rounded-[2px] bg-[var(--c-surface-2)]" />
                <div className="w-[11px] h-[11px] rounded-[2px] bg-[var(--c-border-2)]" />
                <div className="w-[11px] h-[11px] rounded-[2px] bg-[var(--c-line)]" />
                ...
                <div className="h-48">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={ratings}>
                      ...
                      <XAxis
                        dataKey="date" stroke="#3a3a42" fontSize={10}
                      ...
                      />
                      <Line type="monotone" dataKey="newRating" stroke="#8a8a98" strokeWidth={2} dot={{ r: 3, fill: '#9a9aa4' }} name="Rating" />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              </div>
            ...
            <div className="border border-[var(--c-border-2)] rounded-xl bg-[var(--c-surface)]">
              <div className="flex border-b border-[var(--c-border-2)] px-4">
                {([['recent', 'Recent AC'], ['contests', 'Contests'], ['skills', 'Skills']] as [Tab, string][]).map(([key, label]) => (
                  <button
                    ...
                    className={`px-4 py-3 text-sm font-medium transition-colors ${
                      tab === key
                        ? 'text-orange-400 border-b-2 border-rf-pink'
                        : 'text-rf-gray hover:text-[var(--c-fg)]'
                    }`}
                  >
                    {label}
                  </button>
                  ...
                  <div className="space-y-1">
                    {stats.recentAC.length === 0 ? (
                      ...
                      stats.recentAC.map((p: any) => (
                        <div key={p.slug} className="flex items-center justify-between py-2 border-b border-[var(--c-border-2)]/50 last:border-0">
                          <Link
                ...
                {tab === 'contests' && (
                  <div className="space-y-1">
                    {!contests?.length ? (
                      ...
                      contests.map((c: any) => (
                        <div key={c.contestId} className="flex items-center justify-between py-2 border-b border-[var(--c-border-2)]/50 last:border-0">
                          <Link
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            {c.ratingChange !== null && (
                              <span className={`text-xs font-bold ${c.ratingChange >= 0 ? 'text-orange-400' : 'text-red-400'}`}>
                    ...
                    {stats.skills.length === 0 ? (
                      ...
                      <div className="flex flex-wrap gap-2">
                        {stats.skills.map((s: any) => (
                          <span
                            key={s.name}
                            className="px-3 py-1.5 text-xs bg-[var(--c-surface-3)] text-orange-400 rounded-lg border border-rf-iron"
                          >
...
function StatRow({ label, value, color }: { label: string; value: string; color?: string }) {
  return (
    <div className="flex items-center justify-between">
      ...
      <span className={`text-sm font-bold ${color || 'text-orange-400'}`}>{value}</span>
    </div>
  );
}

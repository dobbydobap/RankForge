    ...
    <div className="space-y-6 overflow-y-auto pr-2">
      <div>
        <div className="flex items-center gap-3 mb-2">
          <h1 className="text-xl font-bold text-[var(--c-fg)]">{problem.title}</h1>
        ...
        <div className="flex flex-wrap gap-1.5 mt-2">
          {problem.tags.map((t) => (
            <span key={t} className="px-2 py-0.5 text-xs bg-[var(--c-surface-3)] text-rf-gray rounded">
              {t}
            </span>
          ))}
        </div>
      ...
      {problem.constraints && (
        <div>
          <h3 className="text-sm font-semibold text-[var(--c-fg)] mb-1">Constraints</h3>
      ...
      <div>
        <h3 className="text-sm font-semibold text-[var(--c-fg)] mb-1">Input Format</h3>
        <p className="text-sm text-rf-gray">{problem.inputFormat}</p>
      </div>

      <div>
        <h3 className="text-sm font-semibold text-[var(--c-fg)] mb-1">Output Format</h3>
        <p className="text-sm text-rf-gray">{problem.outputFormat}</p>
      </div>

      {problem.sampleTestCases.length > 0 && (
        <div className="space-y-3">
          <h3 className="text-sm font-semibold text-[var(--c-fg)]">Examples</h3>
...
function DiffBadge({ difficulty }: { difficulty: string }) {
  ...
  const cls = colors[difficulty] || 'bg-[var(--c-surface-3)] text-rf-gray border-rf-iron';
  return (

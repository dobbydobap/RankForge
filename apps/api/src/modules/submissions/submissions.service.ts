...
export class SubmissionsService {
  ...
  async customRun(language: string, sourceCode: string, input: string) {
    const { executeCode } = await import('../judge/executor');
    const result = await executeCode(language, sourceCode, input, 10000);

    if (result.compilationError) {
      return { error: 'Compilation Error', output: result.compilationError.slice(0, 3000) };
    }
    if (result.signal === 'SIGKILL') {
      return { error: 'Time Limit Exceeded', output: '' };
    }
    if (!result.success) {
      return { error: 'Runtime Error', output: (result.stderr || result.stdout).slice(0, 3000) };
    }
    return { error: null, output: result.stdout.slice(0, 10000) };
  }
}
